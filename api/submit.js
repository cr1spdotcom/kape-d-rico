...
//
// Uses the Vercel Functions "fetch" Web Standard export signature so the
// file is auto-detected as a Node.js function from the /api directory.
...
export default {
  async fetch(request) {
    ...
    if (!name || !mobile) {
      return json(400, { ok: false, error: "Name and mobile are required" });
    }
    if (!looksLikePhone(mobile)) {
      return json(400, { ok: false, error: "Please enter a valid mobile number" });
    }
    ...
    try {
      ...
      if (!contactId) {
        ...
        return json(502, { ok: false, error: "Unexpected response from GoHighLevel" });
      }
    } catch (err) {
      console.error("Create contact error:", err);
      return json(500, { ok: false, error: "Failed to reach GoHighLevel" });
    }
